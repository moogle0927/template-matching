...
#ifndef __TEMPLATE_MATCHONG2__
#define __TEMPLATE_MATCHONG2__
...
void template_mathcing_2(const nari::vector<T> &imgRef, const nari::vector<M> &imgFl, const nari::vector<A> &imgAns, const nari::vector<nari::vector<int>> &DispRef,
		...
		for (int k = 0; k < rangez * 2 + 1; k++) {
			for (int j = 0; j < rangey * 2 + 1; j++) {
				for (int i = 0; i < rangex * 2 + 1; i++) {
					int x_p = DispRef[a][0] - rangex + i;
					int y_p = DispRef[a][1] - rangey + j;
					int z_p = DispRef[a][2] - rangez + k;
					if ((x_p >= 0) && (y_p >= 0) && (z_p >= 0)) {
						int in = xeFl*yeFl*(DispRef[a][2] - rangez + k) + xeFl*(DispRef[a][1] - rangey + j) + (DispRef[a][0] - rangex + i);
						if (imgAns[in] == 1) {
							int in_1 = xeFl*yeFl*(DispRef[a][2] - rangez + k - 1) + xeFl*(DispRef[a][1] - rangey + j) + (DispRef[a][0] - rangex + i);
							int in_2 = xeFl*yeFl*(DispRef[a][2] - rangez + k + 1) + xeFl*(DispRef[a][1] - rangey + j) + (DispRef[a][0] - rangex + i);
							int in_3 = xeFl*yeFl*(DispRef[a][2] - rangez + k) + xeFl*(DispRef[a][1] - rangey + j - 1) + (DispRef[a][0] - rangex + i);
							int in_4 = xeFl*yeFl*(DispRef[a][2] - rangez + k) + xeFl*(DispRef[a][1] - rangey + j + 1) + (DispRef[a][0] - rangex + i);
							int in_5 = xeFl*yeFl*(DispRef[a][2] - rangez + k) + xeFl*(DispRef[a][1] - rangey + j) + (DispRef[a][0] - rangex + i - 1);
							int in_6 = xeFl*yeFl*(DispRef[a][2] - rangez + k) + xeFl*(DispRef[a][1] - rangey + j) + (DispRef[a][0] - rangex + i + 1);
							if ((imgAns[in_1] == 0) || (imgAns[in_2]) || (imgAns[in_3]) || (imgAns[in_4] == 0) || (imgAns[in_5]) || (imgAns[in_6])) {
